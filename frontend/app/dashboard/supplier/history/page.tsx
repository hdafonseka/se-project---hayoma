...
import { Search, Filter, ArrowUpDown, Eye, TruckIcon, DollarSign, CheckCircle, Bell, AlertCircle } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const mockAvailabilityNotifications = [
  ...
  },
  {
    ...
    adminResponse: "We'll take it. Please coordinate with logistics for delivery.",
  ...
  },
  {
    ...
    adminResponse: "We currently have sufficient stock. Will contact you for the next order.",
...
export default function SupplierHistoryDashboard() {
  ...
  const [availabilityNotifications, setAvailabilityNotifications] = useState(mockAvailabilityNotifications)
  ...
  const acceptedNotifications = availabilityNotifications.filter((notif) => notif.status === "ACCEPTED").length
  const pendingNotifications = availabilityNotifications.filter((notif) => notif.status === "PENDING").length
  const rejectedNotifications = availabilityNotifications.filter((notif) => notif.status === "REJECTED").length
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Track your supply history, payments, and availability notifications</p>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="supplies">Supply History</TabsTrigger>
          <TabsTrigger value="notifications">Availability Notifications</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="notifications" className="space-y-6 mt-6">
          <div className="grid gap-6 md:grid-cols-4">
            <Card className="bg-gradient-to-br from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 border-blue-100 dark:border-gray-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium">Total Notifications</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{totalNotifications}</div>
                <p className="text-sm text-muted-foreground">All availability notifications</p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-green-50 to-white dark:from-gray-900 dark:to-gray-800 border-green-100 dark:border-gray-700">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-green-600 dark:text-green-400">{acceptedNotifications}</div>
                <p className="text-sm text-muted-foreground">Accepted by admin</p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-amber-50 to-white dark:from-gray-900 dark:to-gray-800 border-amber-100 dark:border-gray-700">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-amber-600 dark:text-amber-400">{pendingNotifications}</div>
            ...
            <Card className="bg-gradient-to-br from-red-50 to-white dark:from-gray-900 dark:to-gray-800 border-red-100 dark:border-gray-700">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-red-600 dark:text-red-400">{rejectedNotifications}</div>
          ...
          <Card className="border-blue-100 dark:border-gray-700">
            <CardHeader>
              ...
              <CardDescription>Track the status of your product availability notifications</CardDescription>
              <div className="flex flex-col sm:flex-row gap-4 mt-4">
                <div className="relative flex-1">
                  <Search className="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input
                ...
                <Table>
                  <TableHeader className="bg-blue-50 dark:bg-gray-900">
                    <TableRow>
                      ...
                      <TableHead>Quantity</TableHead>
                      <TableHead className="cursor-pointer" onClick={() => requestSort("availableDate")}>
                        <div className="flex items-center gap-1">
                          Available Date
                          <ArrowUpDown className="h-3 w-3" />
                        </div>
                      </TableHead>
                      <TableHead className="cursor-pointer" onClick={() => requestSort("sentDate")}>
                        <div className="flex items-center gap-1">
                          Sent Date
                          <ArrowUpDown className="h-3 w-3" />
                        </div>
                  ...
                  <TableBody>
                    {filteredNotifications.map((notification) => (
                      <TableRow key={notification.id} className="bg-white dark:bg-gray-950">
                        <TableCell className="font-medium">{notification.id}</TableCell>
                        <TableCell>{notification.product}</TableCell>
                        <TableCell>{notification.quantity}</TableCell>
                        <TableCell>{new Date(notification.availableDate).toLocaleDateString()}</TableCell>
          ...
          {currentNotification && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>Product availability notification details</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <p>{new Date(currentNotification.availableDate).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground">Sent Date</h3>
                    <p>{new Date(currentNotification.sentDate).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground">Status</h3>
                  ...
                  <div>
                    ...
                    <p className="font-medium">${currentNotification.price.toFixed(2)}</p>
                  </div>
                </div>
                <div>
                ...
                {currentNotification.status !== "PENDING" && (
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground">{currentNotification.adminResponse}</p>
                    <p className="text-xs text-muted-foreground mt-2">
                      Response received on {new Date(currentNotification.responseDate).toLocaleDateString()}
                    </p>
                  </div>
                )}
                ...
                {currentNotification.status === "ACCEPTED" && (
                  <div className="rounded-md border p-4 bg-blue-50 dark:bg-blue-950 border-blue-100 dark:border-blue-900">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Your product availability has been accepted. Please prepare for delivery on{" "}
                      {new Date(currentNotification.availableDate).toLocaleDateString()}. Check the Deliveries tab for
                      pickup details.
                    </p>
                  </div>
                )}
