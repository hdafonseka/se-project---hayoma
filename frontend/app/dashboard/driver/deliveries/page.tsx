...
import { Search, Filter, ArrowUpDown, CheckCircle, Clock, TruckIcon, MapPin, Calendar, DollarSign } from "lucide-react"
...
const mockDeliveries = [
  {
    ...
    stops: [
      ...
      { id: "stop3", name: "Milk & More", address: "789 Pine Blvd", time: "11:00 AM", status: "COMPLETED" },
    ],
    totalDistance: "28 km",
  },
  {
    ...
    paymentAmount: 320.5,
    stops: [
      { id: "stop4", name: "Farm Fresh Dairy", address: "101 Country Rd", time: "02:30 PM", status: "COMPLETED" },
      { id: "stop5", name: "Cheese Masters", address: "202 Dairy Lane", time: "04:00 PM", status: "IN_PROGRESS" },
  ...
  },
  {
    ...
    stops: [
      { id: "stop7", name: "Organic Dairy Shop", address: "404 Green St", time: "07:30 AM", status: "PENDING" },
      { id: "stop8", name: "Premium Dairy", address: "505 Quality Ave", time: "09:00 AM", status: "PENDING" },
      { id: "stop9", name: "Dairy Express", address: "606 Fast Blvd", time: "10:15 AM", status: "PENDING" },
    ],
    totalDistance: "32 km",
  },
  {
    ...
    paymentAmount: 510.0,
    stops: [
      { id: "stop10", name: "Milk Haven", address: "707 Cream St", time: "01:30 PM", status: "PENDING" },
      { id: "stop11", name: "Dairy Corner", address: "808 Butter Ave", time: "03:00 PM", status: "PENDING" },
      { id: "stop12", name: "Fresh Farms", address: "909 Yogurt Blvd", time: "04:15 PM", status: "PENDING" },
    ],
    totalDistance: "30 km",
  },
  {
    ...
    paymentAmount: 325.5,
    stops: [
      { id: "stop13", name: "Dairy Depot", address: "111 Cheese St", time: "08:30 AM", status: "COMPLETED" },
      { id: "stop14", name: "Milk Market", address: "222 Dairy Ave", time: "10:00 AM", status: "COMPLETED" },
      { id: "stop15", name: "Cream & Co", address: "333 Milk Blvd", time: "11:15 AM", status: "COMPLETED" },
    ],
    totalDistance: "25 km",
  },
]
...
export default function DriverDeliveriesDashboard() {
  ...
  const handlePaymentSubmit = () => {
    ...
    const updatedDeliveries = deliveries.map((delivery) => {
      if (delivery.id === currentDelivery.id) {
        return {
          ...
          paymentAmount: Number.parseFloat(paymentDetails.amount) || delivery.paymentAmount,
        }
      }
      return delivery
    })
    ...
    setDeliveries(updatedDeliveries)
    toast.success("Payment status updated and notification sent to admin")
  ...
  if (statusFilter !== "all") {
    filteredDeliveries = filteredDeliveries.filter((delivery) => delivery.status === statusFilter)
  }
  ...
  if (dateFilter !== "all") {
    ...
    if (dateFilter === "today") {
      filteredDeliveries = filteredDeliveries.filter((delivery) => delivery.date === today)
    } else if (dateFilter === "tomorrow") {
      filteredDeliveries = filteredDeliveries.filter((delivery) => delivery.date === tomorrow)
  ...
  const inProgressDeliveries = deliveries.filter((delivery) => delivery.status === "IN_PROGRESS").length
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <Card className="bg-gradient-to-br from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 border-blue-100 dark:border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg font-medium">Total Deliveries</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{totalDeliveries}</div>
            <p className="text-sm text-muted-foreground">All assigned deliveries</p>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-green-50 to-white dark:from-gray-900 dark:to-gray-800 border-green-100 dark:border-gray-700">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-600 dark:text-green-400">{completedDeliveries}</div>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 border-blue-100 dark:border-gray-700">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{inProgressDeliveries}</div>
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-white dark:from-gray-900 dark:to-gray-800 border-amber-100 dark:border-gray-700">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-amber-600 dark:text-amber-400">{pendingDeliveries}</div>
      ...
      <Card className="border-blue-100 dark:border-gray-700">
        <CardHeader>
          ...
          <CardDescription>View and manage your assigned delivery routes</CardDescription>
            ...
            <Table>
              <TableHeader className="bg-blue-50 dark:bg-gray-900">
                <TableRow>
                  ...
                  <TableHead className="cursor-pointer" onClick={() => requestSort("route")}>
                    <div className="flex items-center gap-1">
                      Route
                      <ArrowUpDown className="h-3 w-3" />
                    </div>
                        ...
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const nextStop = delivery.stops.find((stop) => stop.status !== "COMPLETED")
                              if (nextStop) {
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      {delivery.route} <Badge className="bg-blue-500">In Progress</Badge>
                    </CardTitle>
                    <CardDescription>
                      {new Date(delivery.date).toLocaleDateString()} • {delivery.startTime} - {delivery.endTime} •{" "}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {delivery.stops.filter((stop) => stop.status === "COMPLETED").length} of {delivery.stops.length}{" "}
                      stops completed
                    </span>
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-lg border border-blue-100 dark:border-blue-800">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Amount: ${delivery.paymentAmount.toFixed(2)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {getPaymentStatusBadge(delivery.paymentStatus)}
                      {delivery.paymentStatus !== "PAID" && (
                        <Button size="sm" onClick={() => handleOpenPaymentDialog(delivery)} className="ml-2">
                          Update Payment
                        </Button>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        ...
                        {stop.status !== "COMPLETED" && (
                          <Button
                            size="sm"
                            onClick={() => handleCompleteStop(delivery.id, stop.id)}
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Update the payment status for this delivery and notify the admin.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div>
              ...
              >
                <SelectTrigger>
                  <span>{paymentDetails.method || "Select payment method"}</span>
                </SelectTrigger>
                <SelectContent>
